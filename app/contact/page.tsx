            ...
            <main className="pt-24 pb-20">
                ...
                <div className="bg-[#F8FAFC] py-20 border-b border-gray-100">
                    <div className="container mx-auto px-6 max-w-6xl">
                        ...
                        {/* Inquiry Selection Boxes */}
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-16">
                            {inquiryTypes.map((type) => (
                                <button
                                    key={type.id}
                                    onClick={() => handleTypeChange(type.id, type.label)}
                                    className={`group flex flex-col items-center justify-center p-10 rounded-2xl border-2 transition-all duration-500 ${selectedType === type.id
                                ...
                                >
                                    <div className={`p-4 rounded-xl mb-6 transition-colors ${selectedType === type.id ? 'bg-white/10' : 'bg-primary/5 group-hover:bg-primary/10'
                                        }`}>
                                        <type.icon size={32} className={selectedType === type.id ? 'text-secondary' : 'text-primary'} />
                                    </div>
                                    <span className="text-lg font-bold tracking-tight">{type.label}</span>
                                    <div className={`mt-4 w-6 h-1 rounded-full bg-secondary transition-all ${selectedType === type.id ? 'opacity-100 w-12' : 'opacity-0'
                                        }`} />
                                </button>
                            ))}
                        </div>
                        ...
                        >
                            <AnimatePresence mode="wait">
                                {status === "success" ? (
                                    ...
                                    >
                                        <div className="mb-10">
                                            <h3 className="text-2xl font-bold text-primary mb-2">Connect with our team</h3>
